  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', border: `1px solid ${T.bgLine}`, marginBottom: 1 }}>
        <MetricCard label="Final Value" value={`Rs ${(p.total_value || 1000000).toLocaleString()}`} color="amb" />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <div style={{ background: T.bgPanel, border: `1px solid ${T.bgLine}`, padding: 14, flex: 1 }}>
            <div style={{ fontSize: 10, color: T.textDim, letterSpacing: 1.5, marginBottom: 10, textTransform: 'uppercase' }}>
              Trade Summary
              <Link to="/trades" style={{ float: 'right', color: T.amber, letterSpacing: 0 }}>VIEW ALL →</Link>
            </div>
            {[
          ...
          <div style={{ display: 'flex', gap: 1 }}>
            <MetricCard label="Peak Value" value={`Rs ${(p.peak_value || 0).toLocaleString()}`} color="amb" />
